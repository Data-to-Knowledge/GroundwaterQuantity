...
man_sites1 = man_summ_data[(man_summ_data.ToDate >= recent_date) & (man_summ_data.Count >= min_count) & (man_summ_data.FromDate <= first_date) | (man_summ_data.ExtSiteID.isin(input_sites))].copy()
man_sites1 = man_sites1[~man_sites1.ExtSiteID.isin(exclude_sites)].copy()
...
for g in man_sites2.ExtSiteID.tolist():
    ...
    if not near_rec_sites.empty:
        ## Extract all ts data
        g_data = mssql.rd_sql(server, database, ts_daily_table, ['ExtSiteID', 'DateTime', 'Value'], where_in={'DatasetTypeID': man_datasets, 'ExtSiteID': [g], 'QualityCode': qual_codes}, from_date=data_cutoff_date, date_col='DateTime')
        g_data.DateTime = pd.to_datetime(g_data.DateTime)

        r_data = mssql.rd_sql(server, database, ts_daily_table, ['ExtSiteID', 'DateTime', 'Value'], where_in={'DatasetTypeID': man_datasets, 'ExtSiteID': near_rec_sites.index.tolist(), 'QualityCode': qual_codes}, from_date=data_cutoff_date, date_col='DateTime')
